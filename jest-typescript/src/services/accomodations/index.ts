...
import { accomodationHost, hostOnly } from "../../auth/host";

import Accomodation from "./schema";
...
accomodationsRouter.get("/", JWTMiddleware, async (req: any, res: Response, next: NextFunction) => {
	try {
...
accomodationsRouter.put(
	...
	async (req: any, res: Response, next: NextFunction) => {
		try {
			...
			const modifiedAccommodation = await Accomodation.findOneAndUpdate(
				{ _id: accomId, user: { _id: req.user._id } },
				req.body,
...
accomodationsRouter.delete(
	...
	async (req: any, res: Response, next: NextFunction) => {
		try {
			const accomId = req.params.id;
			const deletedAccommodation = await Accomodation.findOneAndDelete({
				_id: accomId,
